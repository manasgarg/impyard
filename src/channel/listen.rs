...
pub async fn supervised(worker: String, platform: String, credential: String) {
    ...
    loop {
        match listen_worker(&worker, &platform, &credential).await {
            Ok(()) => eprintln!("listener {worker}/{platform}: disconnected; reconnecting in {backoff}s"),
...
pub async fn listen_worker(worker: &str, platform: &str, credential: &str) -> Result<(), BErr> {
    ...
    let cred = crate::credential::vault::get_credential(credential)
        .ok_or_else(|| format!("no \"{credential}\" credential in the vault — run: roster server vault connect {platform}"))?;
